...
<nav class="navbar navbar-expand-lg navbar-dark ftco_navbar bg-dark ftco-navbar-light" id="ftco-navbar">
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-md-4">
        <h3 class="text-center text-info">Thêm loại nước</h3>
        <form action="drink.php" method="post" enctype="multipart/form-data">
          <input type="hidden" name="idnuoc" value="<?= $idnuoc; ?>">
          <div class="form-group">
            <input type="text" name="tennuoc" value="<?= $tennuoc; ?>" class="form-control" placeholder="Tên loại nước" required>
          </div>
          <div class="form-group">
            <input type="thongtinnuoc" name="thongtinnuoc" value="<?= $thongtinnuoc; ?>" class="form-control" placeholder="Mô tả" required>
          </div>
          <div class="form-group">
            <input type="tel" name="giatiennuoc" value="<?= $giatiennuoc; ?>" class="form-control" placeholder="Giá tiền" required>
          </div>
          <div class="form-group">
            <input type="hidden" name="oldimage" value="<?= $photonuoc; ?>">
            <input type="file" name="image" class="custom-file">
            <img src="<?= $photonuoc; ?>" width="120" class="img-thumbnail">
          </div>
          <div class="form-group">
            <?php if ($update == true) { ?>
      ...
      <div class="col-md-8">
        ...
        ?>
        <h3 class="text-center text-info">Các loại nước uống</h3>
        <table class="table table-hover" id="data-table">
          ...
          <tbody>
            ...
            <tr>
              <td><?= $row['idnuoc']; ?></td>
              <td><img src="<?= $row['photonuoc']; ?>" width="25"></td>
              <td><?= $row['tennuoc']; ?></td>
              <td><?= $row['thongtinnuoc']; ?></td>
              <td><?= $row['giatiennuoc']; ?></td>
              <td>
                <a href="details-drink.php?details=<?= $row['idnuoc']; ?>" class="badge badge-primary p-2">Thông tin</a> |
                <a href="drink.php?delete=<?= $row['idnuoc']; ?>" class="badge badge-danger p-2" onclick="return confirm('Bạn có muốn xóa cái này?');">Xóa</a> |
                <a href="quanly-nuocuong.php?edit=<?= $row['idnuoc']; ?>" class="badge badge-success p-2">Sửa</a>
              </td>
            </tr>
            <?php } ?>
          </tbody>
